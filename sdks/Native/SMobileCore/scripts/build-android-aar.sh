...
WRAPPER="$REPO_ROOT/sdks/Native/SMobileCore/android/gradlew"
LOCAL_MAVEN="$ANDROID_DIR/build/maven"
DIST_MAVEN="$MODULE_ROOT/dist/android/maven"
FINXCORE_NATIVE_SDK="$REPO_ROOT/sdks/ReactNative/SMobileCoreRN/native-sdk/android/maven"
...
RELATIVE="${ARTIFACT_DIR#${LOCAL_MAVEN}/}" # e.g. com/smobile/smobilecore/0.1.0
VERSION="${RELATIVE##*/}"
GROUP_AND_ARTIFACT="${RELATIVE%/*}"        # com/smobile/smobilecore
ARTIFACT_ID="${GROUP_AND_ARTIFACT##*/}"    # smobilecore
GROUP_PATH="${GROUP_AND_ARTIFACT%/*}"      # com/smobile
...
# Sync to SMobileCore's bundled native-sdk so Nitro modules consume without extra setup
