...
final class SMobileCoreRN: HybridSMobileCoreRNSpec {
  private let keychain = SMobileKeychain(service: "com.smobile.core.keychain")
  private typealias NativeBiometryType = SMobileCore.SMobileBiometryType
  ...
  func biometricsAvailability() throws -> BiometricAvailability {
    let availability = SMobileBiometrics.availability()
  ...
  func authenticateBiometrics(
    ...
    var captured: SMobileBiometricAuthResult?
  ...
  func cryptoRandomBytesBase64(length: Double) throws -> String {
    ...
    let data = try SMobileCrypto.randomBytes(count: count)
  ...
  func cryptoHmacSha256(text: String, key: String) throws -> String {
    return SMobileCrypto.hmacSHA256Hex(data: Data(text.utf8), key: Data(key.utf8))
  }
  ...
  func cryptoAesGcmEncrypt(
    ...
    guard let keyData = Data(base64Encoded: keyBase64) else {
      throw SMobileCryptoError.invalidKey
    }
    let aadData = aadBase64.flatMap { Data(base64Encoded: $0) }

    let result = try SMobileCrypto.aesGCMEncrypt(
  ...
  func cryptoAesGcmDecrypt(
    ...
    let aadData = aadBase64.flatMap { Data(base64Encoded: $0) }
    let plaintext = try SMobileCrypto.aesGCMDecrypt(
  ...
  private func mapBiometryType(_ type: NativeBiometryType) -> SMobileBiometryType {
    switch type {
